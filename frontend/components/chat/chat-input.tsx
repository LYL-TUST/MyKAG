...
export function ChatInput({
  ...
  return (
    <div className="relative">
      ...
      <div className="relative border-t border-border/60 bg-background/90 backdrop-blur-xl">
        <div className="w-full max-w-4xl mx-auto px-3 sm:px-4 py-2">
          ...
          {queuedMessages.length > 0 && (
            <div className="mb-2 space-y-1.5">
              {queuedMessages.map((msg) => (
                <div
                  key={msg.id}
                  className="flex items-center gap-2 px-3 py-2 bg-white/80 border border-border/60 rounded-xl text-sm shadow-sm"
                >
          ...
          <div className="relative group">
            ...
            <div className="relative">
              ...
              <div
                className={`relative bg-card/98 backdrop-blur-sm border-2 rounded-2xl shadow-[0_18px_45px_rgba(95,181,138,0.08)] transition-all duration-300 group-hover:shadow-[0_22px_55px_rgba(95,181,138,0.12)] group-hover:bg-card group-focus-within:shadow-[0_22px_55px_rgba(95,181,138,0.12)] group-focus-within:bg-white/95 group-focus-within:ring-2 group-focus-within:ring-primary/15 ${
                  isDragging
                    ? 'border-primary bg-primary/5 ring-2 ring-primary/20'
                    : 'border-border/60 group-hover:border-primary/45 group-focus-within:border-primary/55'
                }`}
                ...
                <div className="flex items-end gap-2 px-3 py-1.5">
                  ...
                  {!isLoading && (
                    <Button
                      ...
                      className="group h-9 w-9 p-0 mb-0.5 rounded-full bg-muted/60 hover:bg-primary/12 text-muted-foreground hover:text-primary border border-border/60 flex-shrink-0 transition-all duration-200 hover:scale-105 active:scale-95 shadow-sm"
                      type="button"
                  ...
                  {isLoading ? (
                    <Button
                      ...
                      size="sm"
                      disabled={isStopping}
                      className={`h-9 px-4 mb-0.5 rounded-full flex-shrink-0 gap-1.5 transition-all duration-200 hover:scale-105 active:scale-95 bg-destructive/10 text-destructive hover:text-destructive hover:bg-destructive/20 border-2 border-destructive/60 shadow-sm ${isStopping ? 'opacity-60 cursor-not-allowed' : ''}`}
                      type="button"
                    ...
                    <Button
                      ...
                      className="h-9 px-4 mb-0.5 rounded-full flex-shrink-0 gap-1.5 transition-all duration-200 hover:scale-105 active:scale-95 bg-primary text-primary-foreground hover:bg-primary/90 shadow-sm disabled:opacity-40 disabled:cursor-not-allowed"
                      type="button"
                      title="发送"
                    >
          ...
          <div className="hidden sm:flex items-center justify-between mt-1 px-2">
            <p className="text-[11px] text-muted-foreground/60">
              <kbd className="px-1 py-0.5 bg-white/80 border border-border/60 rounded text-[10px] font-medium text-foreground/70 shadow-sm">Enter</kbd> to send
              <span className="mx-1">•</span>
              <kbd className="px-1 py-0.5 bg-white/80 border border-border/60 rounded text-[10px] font-medium text-foreground/70 shadow-sm">Shift+Enter</kbd> new line
            </p>
          </div>
        </div>
      </div>
