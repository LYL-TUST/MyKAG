...
export function ChatInterface({
  ...
  // Keep the latest isNewThread value WITHOUT making it a dependency of the
  // history-load effect. If isNewThread were a direct dependency, the flag
  // clearing after the first message would re-run the effect and wipe the
  // just-streamed messages (flashing back to the welcome screen).
  ...
  const isClientReady = !!userId && !!client
  ...
  useEffect(() => {
    ...
    const loadThreadHistory = async () => {
      ...
      try {
        ...
        const state = await client.threads.getState(currentThreadId).catch((err) => {
          ...
          // Browser/network failures usually mean the backend is down or unreachable.
          if (err instanceof TypeError && String(err.message).includes('Failed to fetch')) {
            console.error('Thread state fetch failed because the backend is unreachable')
            uiDispatch({ type: 'SET_LOADING_THREAD', payload: false })
            return null
          }
    ...
    if (pendingSendRef.current && client) {
      const pending = pendingSendRef.current
      pendingSendRef.current = null
      setTimeout(() => {
        setMessages((prev) => [...prev, pending.userMessage])
        processMessage(pending.content, pending.files, pending.userMessage)
      }, 0)
    }

    // Clear the visible message history immediately so a thread switch or new prompt
    // shows a loading/waiting state instead of briefly flashing the previous thread's content.
    setMessages([])
  ...
  const processMessage = useCallback(async (content: string, files: ImageAttachment[], userMessage: Message, assistantMessageId?: string) => {
    uiDispatch({ type: 'START_SEND' })
    shouldInterruptRef.current = false
    hasSentMessageRef.current = threadId

    try {
      const finalAssistantMessageId = assistantMessageId || generateMessageId()
      const { assistantContent } = await processStream(content, finalAssistantMessageId, files)
  ...
  const processQueue = useCallback(async () => {
    ...
    await processMessage(nextMessage.content, nextMessage.files, nextMessage.userMessage, assistantMessageId)
  ...
  useEffect(() => {
    ...
    if (!trimmedMessage || uiState.hasAutoSent || uiState.isLoadingThread || !isClientReady) {
      return
    }
    ...
    if (autoSend) {
      ...
      processMessage(cappedMessage, [], userMessage, assistantMessageId)
  ...
  const handleSend = useCallback(async () => {
    ...
    if (!isClientReady) {
      console.warn('Chat client is not ready yet - queueing send until ready')
      pendingSendRef.current = {
        content: uiState.input,
        files: [...attachedFiles],
        userMessage: createUserMessage(uiState.input),
      }
      return
    }
    ...
    // Show message in chat immediately and create a pending assistant bubble.
    // The stream handler will reuse this bubble and fill in the answer.
    const assistantMessageId = generateMessageId()
    setMessages((prev) => [
      ...prev,
      userMessage,
    ...
    await processMessage(currentInput, currentFiles, userMessage, assistantMessageId)

    // Check if anything was queued while processing
    if (messageQueueRef.current.length > 0) {
      processQueue()
    }
  }, [uiState.input, uiState.isLoading, uiState.isRegenerating, attachedFiles, isClientReady, agentConfig?.model, setInput, setUploadError, clearFiles, processMessage, processQueue])
        ...
        {uiState.isLoadingThread ? (
          <div className="flex-1 flex items-center justify-center px-4 py-8">
            <div className="rounded-2xl border border-border/60 bg-white/80 px-5 py-4 shadow-sm text-center">
              ...
              <p className="text-sm font-medium text-foreground">正在加载个人知识管理助手...</p>
              <p className="mt-1 text-xs text-muted-foreground">请稍候，正在准备当前会话</p>
            </div>
          </div>
        ) : (
          <MessageList
