import { Copy, Check, Settings, RefreshCw, ThumbsUp, ThumbsDown, MessageSquare, ExternalLink, Bot, UserRound } from "lucide-react"
...
const CODE_COLORS = {
  // Background & borders
  blockBackground: '#f5f8f6',
  blockBorder: '#c9d8ce',
  inlineBackground: '#eef3ef',
  inlineBorder: '#c9d8ce',
...
const customTheme = {
  ...
  'at-rule': { color: CODE_COLORS.primaryDark },
  ...
  return (
    <div className="relative group my-4">
      ...
      <button
        ...
        className="absolute top-2 right-2 sm:top-3 sm:right-3 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity duration-200 px-2 sm:px-2.5 py-1 sm:py-1.5 rounded-md text-xs flex items-center gap-1 sm:gap-1.5 backdrop-blur-sm shadow-sm"
        style={{
...
export const MessageItem = memo(function MessageItem({
  ...
  // 10s metadata 加载兜底:LangSmith fetchUsageMetadata / generateShareLink 是
  // fire-and-forget,失败/超时时 usageMetadata 和 shareUrl 永远不更新,
  // 右下角会一直卡"思考中"。超时后强制显示"运行已完成",不再阻塞 UI。
  const [metadataTimedOut, setMetadataTimedOut] = useState(false)
  useEffect(() => {
    const hasMetadata =
      message.usageMetadata != null || message.shareUrl != null
    if (
      !message.isThinking &&
      message.runId &&
      ...
      const t = setTimeout(() => setMetadataTimedOut(true), 10000)
      return () => clearTimeout(t)
    }
    // metadata 一旦加载到(或换了一条新消息)就重置
    if (hasMetadata || message.thinkingDuration == null) {
  ...
  return (
    <>
        ...
        >
          ...
          >
            {message.role === "assistant" ? (
              <div className="flex h-8 w-8 items-center justify-center rounded-full bg-emerald-100 text-emerald-700 ring-1 ring-emerald-200">
                <Bot className="h-4 w-4" />
              </div>
            ) : (
              <div className="flex h-8 w-8 items-center justify-center rounded-full bg-slate-100 text-slate-700 ring-1 ring-slate-200">
      ...
      <div className="flex-1 min-w-0 space-y-2">
        <div
          className={`rounded-2xl px-4 py-3 transition-all duration-150 ease-out border ${
            message.role === "user" ? "bg-white text-black border-border/80 shadow-sm" : "bg-[#f8fbf8] text-foreground border-border/80 shadow-sm"
          }`}
          style={{
        ...
        >
          ...
          {message.role === "assistant" && (message.isThinking || message.thinkingStartTime || (message.thinkingSteps && message.thinkingSteps.length > 0)) && (
            <details open className="mb-3 text-xs">
              <summary className="cursor-pointer flex items-center gap-2 text-black hover:text-black transition-colors">
                {message.isThinking && <div className="w-2 h-2 rounded-full bg-primary animate-pulse" />}
                <span>
                  {message.isThinking ? <AnimatedThinking /> : <span className="font-medium">思考步骤</span>} ({message.thinkingSteps?.length || 0})
                </span>
              ...
              {message.thinkingSteps && message.thinkingSteps.length > 0 && (
                <div className="mt-2 pl-4 space-y-1 text-black font-mono text-[11px]">
          ...
          {message.role === "user" ? (
            isEditing ? (
              <div>
                <Textarea
                  ...
                  className="min-h-[80px] text-sm bg-white border-border/80 text-[#111827] focus-visible:ring-primary/25"
                  autoFocus
                    onKeyDown={(e) => {
              ...
              <div className="space-y-2">
                        ...
                        >
                          {isImage ? (
                            ...
                            <div className="relative h-full w-full">
                              ...
                              />
                              <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-white/95 to-transparent px-2 py-1">
                                <p className="text-xs text-[#111111] truncate" title={fileName}>
                                  {fileName}
                                </p>
                              </div>
                            </div>
                ...
                {message.content && (
                  <p
                    className="text-sm leading-relaxed whitespace-pre-wrap break-words cursor-pointer rounded px-2 py-1 -mx-2 -my-1 transition-colors overflow-wrap break-word text-[#111827]"
            ...
            <div className="relative">
              <div
                className="text-sm leading-relaxed prose prose-sm max-w-none break-words overflow-wrap break-word transition-opacity duration-200 ease-out prose-headings:text-black prose-p:text-black prose-strong:text-black prose-li:text-black prose-table:text-black prose-th:text-black prose-td:text-black"
                style={{
              ...
              {!message.isThinking && message.runId && message.thinkingDuration != null && (
                <div className="flex items-center justify-end gap-2 mt-3 text-xs text-[#111111] font-mono font-medium">
                  {/* 完整 metadata(已加载成功,含 share link) */}
                  {message.usageMetadata != null && message.shareUrl != null ? (
                    <>
                      <span>{(message.thinkingDuration / 1000).toFixed(1)}s</span>
                      <span>•</span>
                      <span>{((message.usageMetadata?.total_tokens ?? 0) / 1000).toFixed(1)}k tokens</span>
                      {message.usageMetadata && (message.usageMetadata.total_cost ?? 0) > 0 && (
                        <>
                          <span>•</span>
                          <span>${(message.usageMetadata.total_cost ?? 0).toFixed(4)}</span>
                        </>
                      )}
                      <a
                    ...
                    /* 兜底:LangSmith 失败/超时,不再卡"思考中",显示简化状态 */
                    <span className="font-sans font-medium text-[#111111]">
                      {(message.thinkingDuration / 1000).toFixed(1)}s • 运行已完成
                    </span>
                  ) : (
                    /* 还在加载:显示"思考中..." */
                    <span className="font-sans font-medium inline-flex items-center gap-0 relative text-[#111111]">
                      <span className="thinking-text-base">思考中</span>
              ...
              {!message.isThinking && message.runId && (message.shareUrl || message.usageMetadata) && (
                <>
                  ...
                  <Button
                    ...
                    className="h-8 px-2 text-xs rounded-md transition-colors duration-150 ease-out text-[#334155] hover:text-[#111111]"
                  >
            ...
            {showCommentInput === message.id && (
              <div className="mt-2 w-full">
                <Textarea
                  ...
                  className="min-h-[60px] text-xs bg-white border-border/80 text-[#111111] focus-visible:ring-primary/25"
                  autoFocus
                  onKeyDown={(e) => {
                    if (e.key === "Enter" && !e.shiftKey) {
                      e.preventDefault()
            ...
            {showToolCalls && message.toolCalls && message.toolCalls.length > 0 && (
              <div className="mt-3 space-y-2">
                {message.toolCalls.map((tool) => (
                  <div
                    key={tool.id}
                    className="px-3 py-2 rounded-lg border border-border/80 bg-[#f7faf8] text-xs shadow-sm"
                  >
            ...
            {message.subgraphOutputs && message.subgraphOutputs.length > 0 && (
              <div className="mt-3">
                <details className="group">
                  <summary className="cursor-pointer text-xs font-semibold text-muted-foreground hover:text-black transition-colors flex items-center gap-2">
                  ...
                  <div className="mt-2 space-y-2">
                    {message.subgraphOutputs.map((subgraph, idx) => (
                      <details
                        ...
                        className="px-3 py-2 rounded-lg border border-primary/20 bg-primary/5 shadow-sm"
                      >
                        <summary className="cursor-pointer flex items-center gap-2 text-xs text-[#111111] hover:opacity-80">
                          ...
                          {subgraph.isComplete && (
                            <span className="text-[10px] text-[#2f7a59]">
                              Complete
                            </span>
                          )}
                        </summary>
