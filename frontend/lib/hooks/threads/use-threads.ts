...
export function useThreads(userId: string | undefined) {
   ...
   */
  const getUserThreads = useCallback(async (id: string, silent = false): Promise<void> => {
    if (!silent) {
      setIsLoading(true)
    }
    try {
      ...
      if (isNetworkFetchError(error)) {
        logger.error('Error fetching threads: backend is unreachable')
      } else {
  ...
  /**
   * Create a new thread on the backend with the given ID.
   * Fixes the "generate UUID then GET 404" infinite loop.
   *
   * @param id - Thread ID to create
   * @param metadata - Optional metadata to attach to the thread
   * @returns Created thread object or null on failure
   */
  const createThread = useCallback(async (
    ...
    try {
      const client = createLangGraphClient(userId)
      const thread = await client.threads.create({ threadId: id, metadata })
      return thread as any as Thread
    } catch (error) {
