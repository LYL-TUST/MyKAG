...
import { resolveAssistantId } from "../../api/langgraph-client"
...
const RETRY_CONFIG = {
  maxRetries: 3, // Keep metadata retries short so the UI doesn't look stalled
  baseDelay: 500, // 0.5 second initial backoff
  initialDelay: 750, // Brief pause before first attempt to let the run settle
}
...
export function useStreamHandler({
      ...
      try {
        ...
        for (let attempt = 0; attempt < RETRY_CONFIG.maxRetries; attempt++) {
          ...
          try {
            ...
            if (errorMessage.includes("404") || errorMessage.includes("Run not found")) {
              console.log("[TraceURL] Run not ready yet, will retry...")
              continue
            }
        ...
        for (let attempt = 0; attempt < RETRY_CONFIG.maxRetries; attempt++) {
          ...
          try {
            ...
            if (run) {
              ...
              // Update immediately even if token counts are still zero so the UI
              // can stop retrying and show the run metadata promptly.
            ...
            if (errorMessage.includes("404") || errorMessage.includes("Run not found") || errorMessage.includes("Failed to fetch run: Failed to fetch")) {
              console.log("[UsageMetadata] Run not ready yet, will retry...")
              continue
            }
  ...
  const processStream = useCallback(
    async (userContent: string, assistantMessageId: string, images?: ImageAttachment[]) => {
      ...
      const agentType = agentConfig?.agentType ?? "router_agent"
      const repos = agentConfig?.repos ?? []

      // langgraph-api v0.12+ requires an assistant_id (not a graph_id) to start
      // a run. Resolve graph_id -> assistant_id before streaming, otherwise the
      // server returns 404 and the stream yields no messages ("未生成回答").
      const assistantId = await resolveAssistantId(client, agentType)
      ...
      // If the placeholder message already exists, keep reusing it instead of creating another one.
      const assistantMessageExists = !!existingMessage
      ...
      if (eventType === "values" && !isSubgraphEvent && data?.messages && Array.isArray(data.messages)) {
        ...
        const looksLikeSubagentResponse =
          finalContent.trim().startsWith('{"subagent') ||
          finalContent.trim().includes('"subagent_type"')
        ...
        // The stream may sometimes skip partial tokens and only emit a final values event,
        // so we do not require hasSeenNewResponse here.
        if (finalContent && hasFinalMessage && !looksLikeSubagentResponse && !assistantContent) {
          assistantContent = finalContent
          lastAssistantChunk = finalContent
      ...
      // EXCEPT run_single: under router mode the final answer is generated by
      // the knowledge_agent subgraph, whose agent-model token stream is the
      // user-facing reply. Without it the answer appears only after the whole
      // graph finishes (30-90s of silence). Allowing it gives progressive
      // token display while other subgraphs stay filtered.
      // Event naming (langgraph-api): "method|namespace", e.g.
      // "messages/partial|run_single" — method first, namespace after "|".
      const isSingleAgentTokenStream =
        (eventType.startsWith("messages/partial|") ||
          eventType.startsWith("messages|")) &&
        eventType.includes("|run_single")
      if (
        ((eventType === "messages/partial" || eventType === "messages") &&
          (!isSubgraphEvent || isSingleAgentTokenStream)) &&
        data
      ) {
        ...
        if (aiChunk?.content) {
          ...
          const normalizedContent = streamedContent.trim()

          // Treat structured JSON payloads as valid assistant output when they contain
          // a user-facing answer instead of filtering them out entirely.
          const looksStructured = normalizedContent.startsWith('{') || normalizedContent.startsWith('[')
          const isLikelySubagentEnvelope =
            normalizedContent.startsWith('{"subagent') ||
            normalizedContent.includes('"subagent_type"')
          ...
          if (streamedContent && !hasPendingToolCalls) {
            // If the chunk is structured JSON, try to extract a user-facing answer field.
            ...
            if (looksStructured) {
              try {
                ...
                if (typeof candidate === "string" && candidate.trim()) {
                  ...
                  displayContent = typeof candidate === "object" ? JSON.stringify(candidate) : String(candidate)
                }
              } catch {
            ...
            // Don't suppress structured outputs unless they are clearly subagent envelopes
            // with no user-facing answer.
            if (!displayContent.trim() || (isLikelySubagentEnvelope && looksStructured && displayContent === streamedContent)) {
              continue
            }

            // Accumulate the streamed content
            assistantContent = displayContent
            lastAssistantChunk = displayContent
    ...
    setMessages((prev) => {
      ...
      const thinkingDuration = existing?.thinkingStartTime
        ? Math.max(0, Date.now() - existing.thinkingStartTime)
        : undefined
      const fallbackContent =
        assistantContent ||
        lastAssistantChunk ||
        existing?.content ||
        (wasInterrupted ? "Response stopped. The agent was interrupted while processing your request." : "(No response generated)")
