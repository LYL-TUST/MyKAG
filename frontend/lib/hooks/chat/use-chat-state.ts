...
export function useChatState(threadId: string) {
  const draftKey = useMemo(() => `${STORAGE_KEYS.DRAFT_PREFIX}${threadId}`, [threadId])

  // Initialize input from localStorage if available
  const initialInput = typeof window !== 'undefined'
    ? (localStorage.getItem(draftKey) || '').slice(0, MAX_INPUT_CHARS)
    : ''
  ...
  const setInput = useCallback((value: string) => {
    ...
    if (typeof window !== 'undefined') {
      if (cappedValue) {
        localStorage.setItem(draftKey, cappedValue)
      } else {
