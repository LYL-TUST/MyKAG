...
function isTransientReadError(errorMessage: string): boolean {
  const lowered = errorMessage.toLowerCase()
  return (
    lowered.includes("404") ||
    lowered.includes("run not found") ||
...
export async function readRun(runId: string): Promise<any> {
  ...
  try {
    ...
    // Run ingestion is eventually consistent; transient read failures are expected.
    // Don't surface them as errors because the caller already retries.
    if (isTransientReadError(errorMessage)) {
      logger.debug(`[LangSmith] Run ${runId} not available yet: ${errorMessage}`)
    } else {
      logger.error(`[LangSmith] Failed to fetch run ${runId}`, error)
    }

    throw new Error(`Failed to fetch run: ${errorMessage}`)
  }
}
