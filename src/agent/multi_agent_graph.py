...
# 闭环最多重检索轮数(防死循环)。2 -> 1(2026-08-21 延迟优化):
# 实测空检索/低命中问题(vault 里没有的通用知识)在 critic 判不充分后会触发
# 第二次全量检索 + 综合,白白多花 20s+ 而结果几乎不变(rewrite 的查询改写对
# 空检索没帮助)。收紧到 1 后:首次检索不满 -> 最多 rewrite 重试 1 轮 -> 出答案,
# multi_agent 延迟近似减半,质量损失可忽略(真正缺的是笔记内容,不是查询措辞)。
MAX_LOOP_ATTEMPTS = 1
