...
async def enrich_run_metadata(
    ...
    """Inject Personal Knowledge Agent metadata into the root run.

    Note: LangGraph always passes ``assistant_id`` as a UUID, not the
    graph name string. The previous `docs_agent`/`knowledge_agent`
    string compare was therefore unreachable. Authorization is handled
    by ctx.user at the auth-on layer; this handler only enriches
    ...
    metadata.setdefault("source_type", "Personal-Knowledge-Agent")
...
async def allow_assistant_modifications(
    ctx: Auth.types.AuthContext, value: Auth.types.AssistantsCreate
):
    # LangGraph Studio needs to be able to read/update assistant records during
    # local development. Keep this permissive here so Studio can connect without
    # being blocked by assistant ACLs.
    return {}


@auth.on.assistants.read
async def allow_assistant_reads(
    ctx: Auth.types.AuthContext, value: Auth.types.AssistantsRead
):
    # Keep assistant reads permissive for Studio connections.
    return {}
