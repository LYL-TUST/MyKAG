...
class GuardrailsMiddleware(AgentMiddleware[GuardrailsState]):
    ...
    def __init__(self, model: str | None = None, block_off_topic: bool = True):
        ...
        if model is None:
            ...
            model_config = next(
                (cfg for cfg in MODELS.values() if cfg.id == model or cfg.key == model),
                None,
            )
            if model_config is None:
                model_config = GUARDRAILS_MODEL
                logger.warning(
                    "Unknown guardrails model '%s'; falling back to %s",
                    model,
                    model_config.id,
                )
        ...
        logger.info(
            "GuardrailsMiddleware initialized with model: %s (%s)",
            model_config.key,
    ...
    async def abefore_agent(
        ...
        except GuardrailsClassificationError as e:
            logger.warning(
                "Guardrails classification failed after retries; allowing request to continue: %s",
                e,
            )
            return None
    ...
    async def _classify_query(self, messages: list) -> GuardrailsDecision:
        ...
        """
        if self._looks_like_supported_nl_query(messages):
            return {
                "decision": "ALLOWED",
                "explanation": "Supported LangChain-related or nutrition-scoped query bypassed guardrails classification.",
            }
    ...
    def _looks_like_supported_nl_query(self, messages: list) -> bool:
        """Fast-path supported natural language queries to reduce classifier failures."""
        # Be intentionally permissive: only clearly off-topic requests should go
        # through the classifier. Normal product/support questions should not be
        # blocked by a flaky guardrails model.
        for msg in reversed(messages):
            ...
            if any(signal in lowered for signal in supported_signals):
                return True
            return True
